...
def discord_message(device_origin, fixed=False):
    ...
    data = {
      "content": "",
      "username": "ATVReboot Alarm!",
      "avatar_url": "https://github.com/ReuschelCGN/pogoicons/blob/main/tools/android_repair_avatar.png?raw=true",
      "embeds": [
        {
          "title": f"ATV {device_origin} neu gestartet!",
          "color": 16711680,
          "author": {
            "name": "RebootMadDevice",
            "url": "https://github.com/ReuschelCGN/RebootMadDevice",
            "icon_url": "https://github.com/ReuschelCGN/pogoicons/blob/main/tools/message_critical.png?raw=true"
          },
           "thumbnail": {
               "url": "https://github.com/ReuschelCGN/pogoicons/blob/main/tools/android_reboot.png?raw=true"
           },
           "fields": [
            {
              "name": "Gerät",
    ...
    if _rmd_data[device_origin]['webhook_id'] == 0:
        data["embeds"][0]["description"] = f"`{device_origin}` sendet keine brauchbaren Daten seit `{calc_past_sec_from_now(_rmd_data[device_origin]['last_seen'])*60}` Minuten mehr!\nNeustart Zähler: `{_rmd_data[device_origin]['reboot_count']}`"
        try:
